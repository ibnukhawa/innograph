...
odoo.define('website_product_comparison_73lines.comparison', function (require) {
    ...
    var ProductFeaturePanel = Widget.extend({
        ...
        init: function() {
            this.product_data = {};
            this.shortlist_product_ids2 = JSON.parse(utils.get_cookie('shortlist_product_ids2') || 'null') || [];
            this.comparelist_product_ids2 = JSON.parse(utils.get_cookie('comparelist_product_ids2') || 'null') || [];
        },
        start: function() {
            var self = this;
            var product_ids = _.union(self.shortlist_product_ids2, self.comparelist_product_ids2);
        ...
        load_products:function(template_ids) {
            var self = this;
            return ajax.jsonRpc('/shop/get_product_data2', 'call', {
    ...
    var ProductComparison = Widget.extend({
        ...
        init: function(parent){
            this.parent = parent;
            this.comparelist_product_ids2 = parent.comparelist_product_ids2;
        ...
        start:function(){
            ...
            $('.oe_website_sale .o_add_compare').click(function (e){
                ...
                if (self.comparelist_product_ids2.length < self.product_compare_limit) {
                    self.add_new_products($(this));
                } else {
                    self.$('.o_comparelist_limit_warning, .o_product_panel_content').show();
                }
            });
            self.display_products(self.comparelist_product_ids2);
        ...
        add_new_products:function($el){
           ...
           var template_id = $el.data('template-id');
            ...
            if (!_.contains(self.comparelist_product_ids2, template_id)) {
                self.comparelist_product_ids2.push(template_id);
        ...
        check_product_category: function(data) {
            ...
            if (category_ids){
                ...
                if (!_.intersection(categories, data.public_categ_ids).length) {
                    ...
                    this.comparelist_product_ids2 = _.without(this.comparelist_product_ids2, data.id);
                    return false;
                }
            }
        ...
        rm_from_comparelist: function(e){
            this.comparelist_product_ids2 = _.without(this.comparelist_product_ids2, $(e.currentTarget).data('template_id'));
        ...
        update_cookie: function(){
            document.cookie = 'comparelist_product_ids2=' + JSON.stringify(this.comparelist_product_ids2) + '; path=/';
            this.update_comparelist_view();
        },
        update_comparelist_view: function() {
            this.$('.o_product_circle').text(this.comparelist_product_ids2.length);
            this.$('.o_comparelist_button').hide();
            if (_.isEmpty(this.comparelist_product_ids2)) {
                this.$('.o_product_panel_empty').show();
                this.$('.o_comparelist_products').hide();
            } else {
                this.$('.o_comparelist_products').show();
                if (this.comparelist_product_ids2.length >=2) {
                    this.$('.o_comparelist_button').show();
                    this.$('.o_comparelist_button a').attr('href', '/shop/compare/?products='+this.comparelist_product_ids2.toString());
                }
            }
        }
    });
